...
SerialPortWrapper.prototype.write = function(chipIndex, command, data, callback) {
  ...
  this._serialPort.flush(function(error) {
    ...
    this._serialPort.write(new Buffer(packet))
  }.bind(this))
}
...
SerialPortWrapper.prototype._emitResponseIfFinished = function() {
  ...
  if((this._readChecksumData & 0xFF) != this._readChecksum) {
    ...
    error = new Error('Checksum failed for command ' + this._readBuffer[0])
  }
