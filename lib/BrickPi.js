...
BrickPi.prototype._updateValues = function() {
  ...
  for (var i = 0; i < this._serialAddresses.length; i++) {
    ...
    this.once('readSensorData', this._onReadSensorValues.bind(this, i))
...
BrickPi.prototype._onData = function(data) {
  if(data[0] == PROTOCOL.READ_SENSOR_VALUES) {
    this.emit('readSensorData', data)
  }
}

BrickPi.prototype._onReadSensorValues = function(counter, data) {
  ...
  if (motor != null) {
    motor._decodeValue(encoderWordLength0, data, startingBitLocation);
  }
  ...
  if (motor != null) {
    motor._decodeValue(encoderWordLength1, data, startingBitLocation);
  }
  ...
  for (var sensorCount = 0; sensorCount < 2; sensorCount++) {
    ...
    if (currentSensor != null) {
      ...
      startingBitLocation = currentSensor._decodeValue(data, startingBitLocation);
    } else {
