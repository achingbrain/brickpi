...
BrickPi.prototype._setupSensors = function() {
  ...
  for (var i = 0; i < this._serialAddresses.length; i++) {
    ...
    sensorData.unshift(0)
    sensorData.unshift(0)
    sensorData.unshift(PROTOCOL.CONFIGURE_SENSORS)
    ...
    for (var sensorCount = 0; sensorCount < 2; sensorCount++) {
      if (this._sensors[i * 2 + sensorCount] == null) {
        sensorData[1 + sensorCount] = 0
      } else {
        sensorData[1 + sensorCount] = this._sensors[i * 2 + sensorCount].getType();
      }
    }

    tasks.push(this._sendToBrickPiWithRetry.bind(this, i, sensorData))
  }
...
BrickPi.prototype._onReadSensorValues = function(data) {
  ...
  var encoderWordLength0 = decodeInt(bitLength, data, startingBitLocation);
  startingBitLocation += 5;  // skip encoder lengths
  var encoderWordLength1 = decodeInt(bitLength, data, startingBitLocation);
  startingBitLocation += 5;  // skip encoder lengths
  var motor = this.motor(counter * 2);

  if (motor != null) {
    motor.decodeValues(encoderWordLength0, data, startingBitLocation);
  }
  ...
  if (motor != null) {
      motor.decodeValues(encoderWordLength1, data, startingBitLocation);
  }
